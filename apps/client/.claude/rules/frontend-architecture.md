...
The web app is organized as **modules**. A module is a bounded domain area (e.g. `example-feature`, an external-integration adapter like `example-integration`, or `shared`). Inside every module, code follows a **smart hook + dumb component** pattern, with strict separation between business logic, UI, and external systems.

`app/` is the composition root — the only place that imports from multiple modules and wires them together (router, providers, integration selection).
...
src/
  ...
  modules/
    example-feature/             ← domain module
      ...
      example-feature.types.ts   (module-scoped, if any)
      example-feature.constants.ts
      example-feature.utils.ts
      example-feature.config.ts  (env-derived runtime config, if any)
      index.ts                   ← public API (mandatory)
    example-integration/         ← external-integration adapter module: implements a port
      services/
      example-integration.utils.ts
      example-integration.types.ts
      example-integration.constants.ts
      index.ts
    shared/
      components/
      hooks/
      providers/
      layout/
      domain/                    ← port interfaces + domain types
...
## Hexagonal: ports and integration adapters

- A port (interface) and core domain types live in **`modules/shared/domain/`**.
- External-integration adapter modules (`example-integration/`, …) implement the port and export a factory: `createExampleIntegration(config): Port`.
- **A domain module never imports a concrete adapter module.** It depends on the port only.
- **`app/` is the composition root**: it imports the chosen adapter, instantiates it, and passes the resulting port implementation into the domain module via context.
- This means: adding a new external integration requires editing `app/` only.

## Module public API

- Each module under `modules/` exposes a **mandatory `index.ts`** at the module root. This is its public API.
- Cross-module imports must go through `index.ts`: `import { ExamplePage } from '@/modules/example-feature'` ✓; `import { useThing } from '@/modules/example-feature/components/thing/use-thing'` ✗.
...
- An integration adapter module's `index.ts` typically exports: the `create<X>(): Port` factory and integration-specific config types.
- **`shared/` has no top-level barrel.** Import from subpaths: `@/modules/shared/hooks/use-is-mobile`, `@/modules/shared/domain/port`. The exception exists because `shared/` is heterogeneous and a single barrel would make it a god-import.
...
- Use `Readonly<T>` / `ReadonlyArray<T>` only when you explicitly want to widen to a domain type (e.g. `ReadonlyArray<Item>`).
- Primitives (`export const MAX_ITEMS = 100`) need no annotation.
...
- Naming: `<scope>.utils.ts` (e.g. `example-panel.utils.ts`, `example-feature.utils.ts`).
...
- A service talks to the network, wraps a port (an integration port, storage), holds module-internal state, or exposes a lifecycle (start/stop/subscribe).
- A service file may export a factory (`createExampleService(port)`) returning an object. **A service never exports React components or hooks.**
...
- **Component-scoped** → colocated with the component (`example-panel/example-panel.utils.ts`).
- **Module-scoped** → at the module root (`example-feature/example-feature.utils.ts`).
...
- **One provider per folder.** Folder name is the kebab form of the provider (`auth/`, `example-flow/`).
...
2. **A domain module never imports a concrete integration adapter module.** It imports the port from `shared/domain/`.
3. **Integration adapter modules import the port from `shared/domain/`.** They never import a domain module.
4. **`app/` may import from any module's public API.** It is the only module allowed to.

These rules are enforced by `import/no-restricted-paths` in `eslint.config.js`. The zone definitions target each domain/integration module folder (`./src/modules/example-feature`, `./src/modules/example-integration`) from anywhere outside the module, with `except: ['./index.ts', './index.tsx']` allowing only the public-API entry. `shared/` is intentionally **not** a zone — its deep imports are first-class. Two extra zones forbid `example-feature/ → example-integration/` and `example-integration/ → example-feature/` regardless of entry path.

The `@/` path alias (`@/* → src/*`) is configured in `tsconfig.app.json`, `vite.config.ts`, and `vitest.config.ts`. Cross-module imports should use the alias (`@/modules/example-feature`, `@/modules/shared/hooks/use-is-mobile`); the lint rule resolves both alias and relative forms via `eslint-import-resolver-typescript`.
